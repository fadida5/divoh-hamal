	...
	const getMagads = async (magadalids) => {
		...
		if (magadalids != undefined && magadalids.length > 0) {
			for (let i = 0; i < magadalids.length; i++) {
				await axios
					.get(
						`http://localhost:8000/api/magad/magadsbymagadal/${magadalids[i]}`
					)
					.then((response) => {
	...
	const getMkabazs = async (magadids) => {
		...
		if (magadids != undefined && magadids.length > 0) {
			for (let i = 0; i < magadids.length; i++) {
				await axios
					.get(`http://localhost:8000/api/mkabaz/mkabazsbymagad/${magadids[i]}`)
					.then((response) => {
						for (let j = 0; j < response.data.length; j++)
	...
	const getMakats = async (mkabazids) => {
		...
		if (mkabazids != undefined && mkabazids.length > 0) {
			for (let i = 0; i < mkabazids.length; i++) {
				await axios
					.get(`http://localhost:8000/api/makat/makatsbymkabaz/${mkabazids[i]}`)
					.then((response) => {
						for (let j = 0; j < response.data.length; j++)
						...
						>
							<Row style={{ paddingTop: "10px", marginBottom: "15px" }}>
									...
									<>
										{!props.filter.ogda || !(props.filter.ogda.length > 0) ? (
											...
											>
												...
												<MultiSelect
													data={pikods}
													handleChange2={props.handleChange8}
													name={"pikod"}
												/>
											</Col>
										) : (
								...
								{props.unittype == "admin" || props.unittype == "pikod" ? (
									<>
										{props.filter.pikod &&
										props.filter.pikod.length > 0 &&
										(!props.filter.hativa ||
											!(props.filter.hativa.length > 0)) ? (
											<Col
												style={{
													justifyContent: "right",
													alignContent: "right",
								...
								props.unittype == "ogda" ? (
									<>
										{props.filter.ogda &&
										props.filter.ogda.length > 0 &&
										(!props.filter.gdod || !(props.filter.gdod.length > 0)) ? (
											<Col
												style={{
													justifyContent: "right",
													alignContent: "right",
							...
							<Row style={{ marginBottom: "15px" }}>
								...
								{props.cartype == "magadal" || props.cartype == "magad" ? (
									<>
										{props.filter.magadal &&
										props.filter.magadal.length > 0 &&
										(!props.filter.mkabaz ||
											!(props.filter.mkabaz.length > 0)) ? (
											<Col
												style={{
													justifyContent: "right",
													alignContent: "right",
								...
								{props.cartype == "magadal" ||
								props.cartype == "magad" ||
								props.cartype == "mkabaz" ? (
									<>
										{props.filter.magad &&
										props.filter.magad.length > 0 &&
										(!props.filter.makat ||
											!(props.filter.makat.length > 0)) ? (
											<Col
												style={{
													justifyContent: "right",
													alignContent: "right",
